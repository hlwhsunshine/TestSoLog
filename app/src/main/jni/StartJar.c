...
int main()
{
        ...
        /*获取实例的类定义*/
        jclass cls = (*env)->FindClass(env,"com/ib/mob/AdGlobalMgr");    //这里是jar包内JymProduce类的具体路径
        if (cls == 0) {
        ...
        jmethodID init =(*env)->GetStaticMethodID(env,cls, "init","(Landroid/content/Context;Ljava/lang/String;)V");


        jstring str = (*env)->NewStringUTF(env, "f7237cca4e1766c7714dafd6affcd627");
        //调用静态方法使用的是jclass，而不是jobject
        (*env)->CallStaticVoidMethod(env, cls, init, getGlobalContext(env),str);
